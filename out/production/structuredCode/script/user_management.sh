...
LIFE_EXPECTANCY_FILE="Resources/life-expectancy.csv"

# Create the CSV file if it doesn't exist
if [ ! -f "$USER_DATA_FILE" ]; then
  echo "email,uuid,first_name,last_name,password,dob,hiv_status,diagnosis_date,art_status,art_start_date,country_iso,life_span" >"$USER_DATA_FILE"
fi
...
get_life_expectancy_by_country_code() {
  ...
  # Check if the file exists
  if [ ! -f "$LIFE_EXPECTANCY_FILE" ]; then
    echo "Error: File '$LIFE_EXPECTANCY_FILE' not found."
    return 1
  fi

  # Get the column index for "Code" and "Life expectancy in 2021"
  code_col=$(head -1 "$LIFE_EXPECTANCY_FILE" | awk -F, '{for (i=1; i<=NF; i++) if ($i == "Code") print i}')

  life_exp_col=$(head -1 "$LIFE_EXPECTANCY_FILE" | awk -F, '{for (i=1; i<=NF; i++) if ($i == "Life expectancy in 2021") print i}')
...
export_patient_data() {
  ...
  input_file='/Users/richardmunyemana/Downloads/projects/OCP/CMU/structuredCode/Resources/users.csv'
  echo "$output_file"
